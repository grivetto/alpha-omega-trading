...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(message)s',
                    handlers=[logging.FileHandler("/home/sergio/.openclaw/workspace/denaro/LEGION_ATOM.log"), logging.StreamHandler()])
logger = logging.getLogger("Legion_ATOM")
...
def add_to_vault(amount):
    ...
    try:
        ...
        logger.info(f"⚖️ LEGION ATOM HA VERSATO: +{amount:.2f}€ IN CASSAFORTE!")
    except: pass
...
def main():
    logger.info("⚔️ LEGION ATOM AVVIATO. Micro-operativo su timeframe lunghi. Zero OOM.")
        ...
        try:
            ...
            if position:
                ...
                if pnl >= 0.02 or pnl <= -0.04: # +2% TP, -4% SL
                    logger.info(f"⚔️ LEGION ATOM CHIUDE OPERAZIONE! PNL: {pnl*100:.2f}%")
                    ...
                    if drop <= -0.035: # Crollo del 3.5% in 10 minuti
                        try:
                            ...
                            if usdt_bal >= TRADE_AMOUNT_USDT:
                                logger.info(f"⚔️ LEGION ATOM ATTACCA IL DROP (-3.5%)! Prezzo: {price}")
                                buy_price = price
                                position = True
                        except: pass
