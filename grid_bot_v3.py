...
class GridBot(DenaroCore):
    ...
    async def init_grid(self, client, current_price, base_eur):
        ...
        for i, bp in enumerate(reversed(buy_prices)):
            ...
            # Ensure minimum notional (5€ for EUR pairs)
            MIN_NOTIONAL = 5.0
            if order_eur < MIN_NOTIONAL:
                order_eur = MIN_NOTIONAL
            if order_eur > self.config['max_total_invested'] - self.state['total_invested']:
                order_eur = self.config['max_total_invested'] - self.state['total_invested']
            if order_eur < MIN_NOTIONAL:
                logger.info(f"⏭ Skipping buy level {bp}€ (need {MIN_NOTIONAL}€ min notional, only {self.config['max_total_invested'] - self.state['total_invested']:.2f}€ budget left)")
                continue
        ...
        try:
            ...
            if asset_free > 0.01:
                min_sell = 5.0 / current_price  # Min 5€ NOTIONAL for EUR pairs
                per_sell = max(0.05, round(min_sell * 1.1, 5))  # 110% to stay above NOTIONAL
                sell_levels = 3
