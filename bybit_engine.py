#!/usr/bin/env python3
"""
BYBIT ENGINE v5 — Production-grade adapter with WebSocket ticker + order book.
Stessa interfaccia di KrakenEngine per compatibilità con DenaroCore.

Differenze da Kraken:
  - Bybit usa USDT (non EUR), pairs tipo DOGE/USDT
  - Post-only: isPostOnly=True in params (non oflags)
  - WS: stream.bybit.com/v5/public/spot
  - Secret normale (non base64)
"""
...
class _BybitWSFeed:
    """Bybit v5 WebSocket — ticker + order book per un symbol."""
    ...
    def start(self) -> None:
        if not _WS_ENABLED:
            log.info("Bybit WS disabled via BYBIT_WS_DISABLE")
            return
        try:
            import threading
            self._running = True
            ...
            t.start()
            log.info("Bybit WS ticker + book feed started")
        except Exception as e:
            log.warning(f"Bybit WS thread failed: {e}")
    ...
    def _run_loop(self) -> None:
        ...
        try:
            ...
            log.warning(f"Bybit WS loop exited: {e}")
    ...
    async def _ws_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if not self._running:
                    break
                log.warning(f"Bybit WS disconnected ({e}) reconnect in {self._reconnect_delay:.0f}s")
...
class BybitEngine:
    """Bybit exchange adapter — interfaccia compatibile con KrakenEngine."""

    def __init__(self, api_key: str, api_secret: str, symbol: str = SYMBOL):
        self._ws = _BybitWSFeed(symbol)
        self._symbol = symbol
        self._last_request: float = 0.0
        self._min_interval: float = 0.15  # 150ms tra richieste
        self._maker_fee = 0.001  # Bybit spot maker
        self._taker_fee = 0.001  # Bybit spot taker
        ...
        # Carica precision dai mercati
        try:
            self.ex.load_markets()
            m = self.ex.market(symbol)
            self._amount_precision = m.get("precision", {}).get("amount", 4)
            if isinstance(self._amount_precision, float):
                self._amount_precision = int(-math.log10(m["precision"]["amount"])) if m["precision"]["amount"] > 0 else 4
            self._price_precision = m.get("precision", {}).get("price", 5)
            if isinstance(self._price_precision, float):
                self._price_precision = int(-math.log10(m["precision"]["price"])) if m["precision"]["price"] > 0 else 5
            self._maker_fee = m.get("maker", 0.001)
            self._taker_fee = m.get("taker", 0.001)
            log.info(f"Bybit markets loaded — {symbol} amount_prec={self._amount_precision} price_prec={self._price_precision}")
        except Exception as e:
            log.warning(f"Bybit load_markets fallback precision: {e}")
    ...
    def create_limit_buy_order(self, symbol: str, amount: float, price: float) -> dict:
        self._throttle()
        try:
            return self.ex.create_limit_buy_order(symbol, amount, price, {"isPostOnly": True})
    ...
    def create_limit_sell_order(self, symbol: str, amount: float, price: float) -> dict:
        self._throttle()
        try:
            log.debug(f"Bybit SELL {amount} {symbol} @ {price}")
            return self.ex.create_limit_sell_order(symbol, amount, price, {"isPostOnly": True})
        except ccxt.InvalidOrder:
