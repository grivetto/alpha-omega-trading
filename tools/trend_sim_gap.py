...
def simula_gap(assets, capitale, risk, fee=FEE, slip_k=SLIP, tetto_capitale=True, gap=0.0, **kw):
    ...
    for i in range(inizio, n):
        ...
        for p in prep:
            ...
            if p["asset"] <= 1e-12:
                continue
            if c[i]["l"] <= p["stop"]:
                prezzo_uscita = p["stop"] * (1.0 - gap)
                sl = E.slippage(p["asset"] * prezzo_uscita, E._media_volumi(c, i), slip_k)
                incasso = p["asset"] * prezzo_uscita * (1.0 - fee - sl)
                cash += incasso
