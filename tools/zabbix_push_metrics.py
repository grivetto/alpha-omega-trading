...
SERVICES = {
    "marcodg1": {
        ...
        "ssh": [],  # locale
        "units": [
            "denaro-node-marcodg1-xrp",    # nodo di TRADING live
            "denaro-dashboard-marcodg1",   # dashboard pubblica (web.grivetto.eu)
            "cloudflared-denaro",          # tunnel dashboard + zab.grivetto.eu
            "denaro-aggregator-marcodg1",
    ...
    "nuvola": {
        "host": "nuvola",
        "ssh": ["sergio@87.106.3.15", "-p", "22"],
        "units": ["denaro-node-nuvola-trade", "denaro-health-nuvola", "zabbix-agent"],
    },
    "mc2": {
        "host": "mc2",
        "ssh": ["sergio@127.0.0.1", "-p", "2222"],  # tunnel inverso
        "units": ["denaro-node-mc2", "denaro-aggregator-mc2", "denaro-dashboard-mc2",
...
def push_remote_nodes(data, auth):
    ...
    for node_name, cfg in REMOTE_NODES.items():
        ...
        # Auto-heal remoto: non riavviare se il nodo ha tutti i bot disabilitati (es. nuvola)
        if all_stale and node_name != "nuvola":
...
def heal_if_stale() -> None:
    ...
    for source, unit in HEAL_UNITS.items():
        ...
        ts = data.get("timestamp", 0) if (data and source.suffix == ".json" and "health" in source.name) else 0
