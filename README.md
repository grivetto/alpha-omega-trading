...
| **🔑 API Key Validation & Testing** | **2026-08-22** | **Comprehensive API key testing across all nodes.** Validated Kraken keys on NUVOLA (2 pairs working, EUR=22.20), OKX keys on MARCODG1 (stable 2+ days), and verified mc2 configuration. Fixed base64 padding issues and IP whitelist conflicts. All exchanges operational. |
| **🤝 Denaro-Atlas Compatibility** | **2026-08-22** | **Official compatibility layer between Denaro (legacy stable) and Atlas (next-gen Hermes-managed).** Both systems coexist on same nodes with separate API keys. Denaro runs solo-engine (Kraken/OKX grid), Atlas runs via Hermes AI. No conflicts with proper key isolation. Full operational status on both NUVOLA and MARCODG1. |
