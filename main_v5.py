...
def main() -> None:
    ...
    if MOCK_MODE:
        ...
        # In SHADOW, prova engine reale ma fallback a mock se key invalide
        try:
            engine = BybitEngine(api_key, api_secret, symbol=SYMBOL)
            log.info("BybitEngine initialized (SHADOW mode)")
        except Exception as e:
            log.warning(f"BybitEngine init failed in SHADOW: {e} — using MockKrakenEngine")
            from mock_runner import MockKrakenEngine
            base_asset = SYMBOL.split("/")[0]
            engine = MockKrakenEngine(initial_eur=CAPITAL, start_price=0.065,
                                      quote_currency=CURRENCY, base_currency=base_asset)
            log.info("MockKrakenEngine fallback (SHADOW mode)")
    else:
        try:
            engine = BybitEngine(api_key, api_secret, symbol=SYMBOL)
        except Exception as e:
