...
# MARCODG1 Monitor Script - Fixed Bash version
SSH_HOST="marco@87.106.222.123"
SSH_KEY="~/.ssh/id_ed25519"
TELEGRAM_TOKEN="8715...qlmw"
...
# Check if volatility is numeric and less than 1.0
if [[ "$VOLATILITY" =~ ^[0-9]+(\.[0-9]+)?$ ]] && [ $(echo "$VOLATILITY < 1.0" | bc -l 2>/dev/null || echo 0) -eq 1 ]; then
    ALERTS="$ALERTS⚠️ Bassa volatilità: ${VOLATILITY}%\n"
    send_alert "Attenzione: volatilità bassa (${VOLATILITY}%) su MARCODG1"
fi

if [ $(echo "$EUR_BALANCE < 1.0" | bc -l 2>/dev/null || echo 0) -eq 1 ]; then
    ALERTS="$ALERTS⚠️ Basso saldo EUR: ${EUR_BALANCE}€\n"
    send_alert "Attenzione: saldo EUR EUR basso (${EUR_BALANCE}€) su MARCODG1"
fi
