...
class WelfordRolling:
    ...
    def push(self, x: float) -> None:
        ...
        if evicted is not None and self.n > self.window:
            delta_old = evicted - self.mean
            mean_old = self.mean
            self.mean -= delta_old / (self.n - 1)
            delta2 = evicted - self.mean
            self.m2 -= delta_old * delta2
            self.n -= 1
...
class LFAMR_VCC:
    ...
    def on_tick(self, market: Any, orders: Any) -> int:
        ...
        self.ewma_var.update(0.0)  # keep ewma clock warm (idempotent)
        signal = self.ewma_var  # reuse object; store smoothed rho in _rho_hist
        # simple EWMA smoothing of rho using existing Ewma-like recursion
        if self._rho_hist:
        ...
        band = self.config["band_mult"] * max(sig, 1e-6)
        if vol_cluster:
            band *= self.config["vol_clamp_band_mult"]  # widen: wait longer to fade

        # (cooldown handled by the exit/trend branch below)
        ...
        if vol_cluster:
            band_pct *= self.config["vol_clamp_band_mult"]
